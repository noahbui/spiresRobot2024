...
CLIMB_SPEED_FUDGE_FACTOR = 0.5 # TODO: this should be between 0 and 1
...
class ClimberMotorControl():
    ...
    def update(self):
        if self.hasZeroed == "no":
            ...
            if self.motor.getMotorVelocityRadPerSec() > -ZERO_OUT_RAD_PER_SEC * 0.05:
                if self.count >= 25: # stopped moving for a half second
                    self.count = 0
                    self.changeHasZeroed("awaitingOtherSide")
                else:
                    self.count += 1
        elif self.hasZeroed == "awaitingOtherSide":
            if self.otherSideIsWaiting:
                self.motor.setVoltage(0.0)
                self.changeHasZeroed("yes")
                self.otherSideIsWaiting = False
            elif self.motor.getMotorVelocityRadPerSec() < -ZERO_OUT_RAD_PER_SEC * 0.05:
                self.changeHasZeroed("zeroing")
        ...
        log(f'Climber Control Zeroed {self.name}', self.hasZeroed == "yes", "bool")
    ...
    def setOtherSideIsWaiting(self, otherIsWaiting: bool):
        self.otherSideIsWaiting = otherIsWaiting
    def isWaiting(self):
        return self.hasZeroed == "awaitingOtherSide"


class ClimberControl(metaclass=Singleton):
    def __init__(self):
        # TODO: these spark max can IDs should be moved to a constants file
    ...
    def update(self):
        ...
        self.motorRight.setOtherSideIsWaiting(self.motorLeft.isWaiting())
        self.motorLeft.setOtherSideIsWaiting(self.motorRight.isWaiting())
